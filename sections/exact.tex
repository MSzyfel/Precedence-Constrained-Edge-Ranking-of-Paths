...
When the structure of the precedence constraints reflects an ordering induced by a proper bracketing of the algebraic expression, the precedence-constrained edge ranking of a path can be solved in $O(n^2)$ time for both the min-max and the min-sum versions of the problem.
\end{theorem}
\begin{proof}
    ...
    \begin{lemma}\label{lemma:separation}
        Let $e$ be the unique edge with the highest color in an optimal precedence-constrained edge ranking of a path. If $\spr{P-e}=2$, then no precedence constraint are present between edges in the two connected components of $P-e$. 
    \end{lemma}
    \begin{proof}
         \MS{TODO: Add proof.}
    \end{proof}
...
\begin{align*}
\OPT\br{i,j} = \min_{i\leq k < j, (k,k+1) \text{ is a maximum element}}\brc{1+\max\brc{\OPT\br{i, k}, \OPT\br{k+1, j}}}
\end{align*}

It is ease to see that the above recurrence relationship can be solved in $O\br{n^3}$ time since there are at most $O\br{n^2}$ subproblems and each subproblem can be solved by checking at most $O\br{n}$ possible choices for the edge with the highest color. We will now show how to speed up the above algorithm to run in $O\br{n^2}$ time. The idea behind the speed-up described below is due to Knuth \cite{Knuth1973} and \cite{EffDPusingQI}. For every $i\leq k< j$ let $\OPT_k\br{i,j} = 1 + \max\brc{\OPT\br{i, k}, \OPT\br{k, j}}$ be the optimal cost of coloring $P[i,j]$ assuming that the edge $\br{k, k+1}$ is the unique edge with the smallest color. We have that $\OPT\br{i,j}\leq \OPT_k\br{i,j}$. Additionally, define 
...
$$
\OPT\br{i,j} = \min_{K\br{i,j-1}\leq k\leq K\br{i +1,j}, (k,k+1) \text{ is a maximum element}}\brc{1+\max\brc{\OPT\br{i, k}, \OPT\br{k+1, j}}}
$$
...
\begin{align*}
\OPT\br{i,j} = \min_{i\leq k < j, (k,k+1) \text{ is a maximum element}}\brc{j-i+\OPT\br{i, k}+ \OPT\br{k+1, j}}
\end{align*}
