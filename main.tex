...
\title{Precedence-Constrained Edge Ranking of Paths} %TODO Please add

%\titlerunning{Dummy short title} %TODO optional, please use if title is longer than one line

\author{Michał Szyfelbein}{Gdańsk University of Technology, Poland}{johnqpublic@dummyuni.org}{https://orcid.org/0000-0002-1825-0097}{(Optional) author-specific funding acknowledgements}%TODO mandatory, please use full name; only 1 author per \author macro; first two parameters are mandatory, other parameters can be empty. Please provide at least the name of the affiliation and the country. The full address is optional. Use additional curly braces to indicate the correct name splitting when the last name consists of multiple name parts.

\author{Dariusz Dereniowski}{Gdańsk University of Technology, Poland}{joanrpublic@dummycollege.org}{[orcid]}{[funding]}
...
\begin{abstract}
We study the problem of precedence-constrained edge ranking of paths, which is a variant of the classical edge ranking problem. In this problem, we want to assign numbers to the edges of a path so that every two edges labeled with the same number have at least one edge with a lower number between them. Additionally, we have precedence constraints that require some edges to be labeled with smaller numbers than the others.
The goal is to find a labeling that satisfies the precedence constraints. We wish to minimize either the number of labels used or the sum of the labels assigned to the edges. 
This problem can be used to model parallel evaluation of algebraic expressions, with precedence constraints induced by the order of operations.
We show that the problem is NP-hard for both criterions, and admits $O(\log n)$-approximation algorithms. In fact we show that when minimizing the number of colors it is NP-hard to approximate the problem within a factor of $O(1+\frac{1}{\log n})$, unless P=NP. This implies that no PTAS for the problem exists unless NP admits quasi-polynomial time algorithms.
...
We start by showing that when the input graph consists of a collections of paths of $O\br{1}$ length the problem is in fact APX-hard. Then, we show how to use a generic csontruction, which transforms an instance of the problem with a collection of paths into an instance of the problem with a single path. This is done by connecting the paths in an arbitrary order, and enforcing that the additional edges are labeled with colors which are levels of a full binary tree of logarithmic height. By doing so, we show that the problem is NP-hard to approximate within a factor of $O(1+\frac{1}{\log n})$, unless P=NP. This implies that no PTAS for the problem exists unless NP admits quasi-polynomial time algorithms.
\begin{theorem}
\label{thm:apx-hardness}
The precedence-constrained edge ranking is APX-hard, even when the input graph consists of a collection of paths of $O\br{1}$ length.
\end{theorem}

\begin{theorem}
\label{thm:np-hardness}
The precedence-constrained edge ranking of a path is NP-hard to approximate within a factor of $O(1+\frac{1}{\log n})$, unless P=NP.
\end{theorem}
...
\begin{theorem}
There exists an $O\br{\log n}$-approximation algorithm for the precedence-contrained edge ranking of a path.
\end{theorem}

\begin{theorem}
There exists an $O\br{\log n}$-approximation algorithm for the precedence-contrained min-sum edge ranking of a path.
\end{theorem}
